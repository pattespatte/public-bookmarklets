...
void function () {
	...
	if (a) {
		for (let d, e = 0; e < a.length; e++) {
			d = "",
				(d = a[e].querySelector("input,select,textarea")) ? (d.required || a[e].innerText.search(/\*/)) && b.push({ labelText: a[e].innerText, label: a[e], field: d }) : a[e].getAttribute("for") && (d = document.getElementById(a[e].getAttribute("for")), (d.required || 0 < a[e].innerText.search(/\*/)) && b.push({ labelText: a[e].innerText, label: a[e], field: d }));
			let f = [...a[e].innerText.matchAll(/\w{2,}/g)];
			f.forEach(a => { c[a[0]] ? c[a[0]]++ : c[a[0]] = 1 })
		}

		for (let d in c) if (2 < c[d]) {
			let c = a.filter(function (a) { return 0 <= a.textContent.search(d) });
			if (0 < c.length) for (let a = 0; a < c.length; a++) b[c[a].innerText] || b.push({ labelText: c[a].innerText, label: c[a], requiredText: d })
		}

		for (let a, c = 0; c < b.length; c++) {
			a = b[c],
				a.requiredText ? (a.label.classList.add("required-label"), a.label.insertAdjacentHTML("afterbegin", "<span class='required-span'>Correctly marked with '" + a.requiredText + "'?</span>")) : (a.label.classList.add("required-label"), a.label.insertAdjacentHTML("afterbegin", "<span class='required-span'>Required field</span>"))
		}
