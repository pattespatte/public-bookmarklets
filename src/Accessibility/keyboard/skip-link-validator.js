// Description: Validates skip links for accessibility - finds all # links, checks if first link is a skip link, validates aria-label/text
(function() {
  'use strict';
  try {
    // Remove existing bookmarklet UI
    const existingHost = document.getElementById('skip-link-validator-host');
    if (existingHost) existingHost.remove();

    // Create shadow DOM host for UI
    const host = document.createElement('div');
    host.id = 'skip-link-validator-host';
    host.style.cssText = 'position:fixed;top:10px;right:10px;z-index:999999;width:400px;max-height:80vh;overflow:auto;';
    ...
    skipLinks.forEach((link, index) => {
      ...
      if (!hasAccessibleName) {
        ...
        linkInfo.issue = 'Skip link lacks accessible name (no visible text, aria-label, or title)';
      ...
      const skipKeywords = ['skip', 'jump', 'main content', 'navigation', 'menu'];
      const isSkipLink = skipKeywords.some(keyword => {
        const text = (visibleText + ' ' + (ariaLabel || '') + ' ' + (title || '')).toLowerCase();
    ...
    if (allLinks.length > 0 && !results.firstLinkIsSkip) {
      results.issues.push('First link in page is not a skip link - keyboard users must tab through all navigation');
    }
    ...
    if (results.skipLinks.length > 0) {
      html += '<div style="font-weight:600;margin-bottom:8px">Skip Links Found:</div>';
      results.skipLinks.forEach(linkInfo => {
        const statusClass = linkInfo.hasIssue ? 'fail' : 'pass';
        let badges = '';
        if (linkInfo.isFirstOverall) badges += '<span class="slv-badge first">First Link</span>';
        if (linkInfo.isSkipLink) badges += '<span class="slv-badge skip">Skip Link</span>';
      ...
      html += '<div style="color:#666;font-style:italic">No skip links found on this page.</div>';
    }
    ...
    globalStyle.textContent = '.slv-highlight { outline: 3px dashed #ffc107 !important; outline-offset: 2px !important; }';
