// Description: Tests for keyboard traps - automated tabbing through focusable elements, detects focus cycles, highlights trapped elements
(function() {
  'use strict';
  try {
    ...
    const existingHighlights = document.querySelectorAll('.ktt-trap, .ktt-visited, .ktt-current');
    ...
    host.style.cssText = 'position:fixed;top:10px;right:10px;z-index:999999;width:420px;max-height:85vh;';
    ...
    window.startKeyboardTrapTest = async function() {
      ...
      if (focusableElements.length === 0) {
        updateStatus('complete', 'No Focusable Elements', 'No focusable elements found on this page.');
        return;
      }
      ...
      updateStatus('testing', 'Testing in Progress', `Found ${focusableElements.length} focusable element(s). Simulating Tab key presses...`);
      ...
      while (iterations < maxIterations) {
        ...
        if (currentElement && currentElement !== document.body) {
          ...
          setTimeout(() => currentElement.classList.remove('ktt-current'), 50);
        }
      ...
      // Clean up highlights
      document.querySelectorAll('.ktt-current').forEach(el => el.classList.remove('ktt-current'));

      // Restore initial focus if possible
      try {
        if (initialActiveElement && typeof initialActiveElement.focus === 'function') {
    ...
    function updateProgress(percent) {
      let progressContainer = shadow.getElementById('ktt-progress-container');
      if (!progressContainer) {
        progressContainer = document.createElement('div');
        progressContainer.id = 'ktt-progress-container';
        progressContainer.innerHTML = '<div class="ktt-progress"><div class="ktt-progress-bar" id="ktt-progress-bar"></div></div>';
        const statusEl = shadow.getElementById('ktt-status');
    ...
    function displayResults(trapInfo, focusableElements) {
      ...
      if (trapInfo.hasTraps) {
        updateStatus('traps-found', 'Keyboard Traps Detected!', `Tested ${trapInfo.visitedCount} element(s) before detecting a cycle.`);

        // Highlight trap elements
        trapInfo.traps.forEach(trap => {
          if (trap.element) trap.element.classList.add('ktt-trap');
          if (trap.cycleStart) trap.cycleStart.classList.add('ktt-trap');
        });

        let resultsHtml = '<div class="ktt-section-title">Traps Found:</div>';
        ...
        updateStatus('complete', 'No Keyboard Traps Found', `Successfully tested through ${trapInfo.visitedCount} element(s) without detecting cycles.`);
    ...
    window.highlightKTTElement = function(itemEl, tagName) {
      document.querySelectorAll('.ktt-visited').forEach(el => el.classList.remove('ktt-visited'));
    ...
    window.cleanupKTT = function() {
      ...
      document.querySelectorAll('.ktt-trap, .ktt-visited, .ktt-current').forEach(el => {
        el.classList.remove('ktt-trap', 'ktt-visited', 'ktt-current');
      });
