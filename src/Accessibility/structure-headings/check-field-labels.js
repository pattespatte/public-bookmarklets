...
(function () {
	try {
		...
		for (let a of document.querySelectorAll("input, textarea, select")) {
			...
			if (a.getAttribute("type") && a.getAttribute("type").match(/hidden|button|submit|reset/)) {
				continue;
			}

			// Check for label with `for` attribute
			let label = a.getAttribute("id")
				? document.querySelector("label[for='" + a.getAttribute("id") + "']")
				: null;
			...
			if (label) {
				...
				let message = a.getAttribute("id")
					? "✗ Missing label (No matching label for ID='" + a.getAttribute("id") + "' found)"
					: "✗ Missing label";

				a.insertAdjacentHTML(
					"beforebegin",
					"<span class='wai-bad'><span class='wai-label'>" + message + "</span></span>"
				);
				a.previousElementSibling.appendChild(a);
			}
		}
