// Description: Validates heading structure - finds all headings h1-h6, checks for proper hierarchy (no skipped levels), detects multiple h1s, displays visual outline
(function() {
  'use strict';
  try {
    ...
    host.style.cssText = 'position:fixed;top:10px;right:10px;z-index:999999;width:450px;max-height:85vh;';
    ...
    headings.forEach((heading, index) => {
      ...
      if (previousLevel > 0 && level > previousLevel + 1) {
        ...
        issues.push({ type: 'error', message: `H${level} "${headingInfo.text}" - ${issue}` });
      }
      ...
      if (level === 1 && headingCounts.h1 > 1) {
        ...
        if (headingCounts.h1 === 2) {
          issues.push({ type: 'warning', message: 'Multiple h1 headings detected' });
        }
      }
    ...
    if (headings.length > 0) {
      ...
      headingOutline.forEach((h, i) => {
        ...
        const statusClass = h.hasIssue ? 'error' : (h.hasWarning ? 'warning' : 'valid');
    ...
    globalStyle.textContent = '.hsv-highlight { outline: 3px solid #007bff !important; outline-offset: 2px !important; background: rgba(0,123,255,0.1) !important; }';
    ...
    window.scrollToHeading = function(index) {
      ...
      document.querySelectorAll('.hsv-highlight').forEach(el => el.classList.remove('hsv-highlight'));
      ...
      if (heading) {
        ...
        setTimeout(() => heading.classList.remove('hsv-highlight'), 2000);
      }
    };
