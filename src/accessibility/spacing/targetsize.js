...
// Description: The Target Size bookmarklet visualizes touch target sizes by drawing 24px circles at the center of each interactive element. Blue circles indicate targets smaller than 24x44px (failing WCAG), green circles indicate passing sizes. Also detects and alerts on overlapping controls. Run again to remove the visualization. WCAG SC 2.5.5: Target Size.
(function () {
	...
	elements.forEach((el) => {
		if (!el.matches('label') && el.closest('label')) {
			return;
		}
	...
	centers.forEach((item1, index) => {
		centers.slice(index + 1).forEach((item2) => {
			if (
				Math.sqrt(
					Math.pow(item2.center.left - item1.center.left, 2) +
						Math.pow(item2.center.top - item1.center.top, 2)
				) < 24
			) {
	...
	uniqueOverlaps.forEach((el) =>
		el.setAttribute('aria-description', 'overlap')
	);
...
Bookmarklet name: Target Size (WCAG SC 2.5.8)
`);
