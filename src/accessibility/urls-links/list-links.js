	...
	function listLinks() {
		...
		Array.from(links).forEach(function (link) {
			...
			if (link.getAttribute('aria-label')) {
				...
				notes +=
					'- Link gets accessible name from <code>aria-label</code>';
				if (tc.trim() !== '') {
					notes +=
			...
			if (
				...
				notes +=
					'- Same text but case difference noted (likely not an issue)';
			}
			row += '<td>';
			if (warn) {
				row +=
...
Bookmarklet name: List links
`);

/*
Searches for all real links on the page, and any element given a role=link.
