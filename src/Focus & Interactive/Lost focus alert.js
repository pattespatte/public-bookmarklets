(function () {
	...
	let a = document.createElement("style"), b;
	document.head.appendChild(a);
	(b = a.sheet).insertRule("#focLostNotif:empty {visibility:hidden;}", 0);
	b.insertRule("#focLostNotif {background:black;color:white;padding:1em;font-size:2em;border-radius:10px;outline:4px solid white;position:fixed;bottom:2em;left:50%;transform:translateX(-50%)}", 0);
		...
		t.addEventListener("click", t => {
			...
			delayTimeout = setTimeout(function () {
				...
				if ("BODY" === activeEl.tagName) {
					...
					statusNotif.innerHTML = "FOCUS WAS LOST!<br>Check Console for details";
					clearStatus();
				}
			}, 100);
		});
