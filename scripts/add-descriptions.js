...
function generateDescription(relPath) {
  ...
  const patterns = {
    ...
    'remove-colors': 'Removes all colors from the page, showing only black and white',
    'contrast-checker': 'Opens WebAIM contrast checker to test color contrast',
    'contrast-checker-self-contained': 'Standalone contrast checker bookmarklet',
    'contrast': 'Color contrast analysis tool',
    'lost-focus-alert': 'Alerts when focus is lost from the active element',
    'reveal-focus-order': 'Visualizes the focus order of interactive elements',
    ...
    'text-spacing-bookmarklet': 'Tests text spacing requirements (WCAG 1.4.12)',
    'font-size-2em-wcag-1.4.4': 'Sets font size to 200% to test WCAG 1.4.4 reflow',
    'check-field-labels': 'Checks form fields for proper labels',
    'landmarks': 'Lists ARIA landmarks on the page',
    'landmarks-accessibility-bookmarklets': 'Comprehensive landmark and accessibility checker',
    'deque-w3c-validator-bookmarklet': 'Opens Deque/W3C validator for the current page',
    ...
    'copy-selection-to-md-with-turndown': 'Copies selected text as Markdown using Turndown',
    'copy-selection-to-md': 'Copies selected text as Markdown',
    'unblock-router': 'Unblocks ASUS router login page',
    'check-language-on-this-page-w3c': 'Checks page language declaration (W3C)',
    'check-required-fields-on-this-page-w3c': 'Checks form required field indicators (W3C)',
...
for (const srcPath of jsFiles) {
  ...
  for (let i = 0; i < Math.min(10, lines.length); i++) {
    ...
    if (lines[i].trim().startsWith('(function') || lines[i].trim().startsWith('function')) {
      insertIndex = i;
      break;
    }
  }
